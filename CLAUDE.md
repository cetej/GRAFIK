...
### Unified Editor — M6-UX1 první iterace z připomínek operátora (HOTOVO, 2026-08-15)
E2E důkaz: `docs/spikes/2026-08-15-m6ux1-e2e.md` (reálný Chrome, ~$0.49 dle ledgerů) · master `49c91e6`…`775317c` · **UX iterace pokračují, sběr připomínek u operátora.**
- [x] **Zvýraznění výběru**: Transformer restyle (#4a9dff, anchory 10 px) + trvalý outline Rect v ne-select módech; hover z panelu vrstev = ostatní vrstvy ×0,25 + čárkovaný obrys (`hoveredPanelLayerId`)
- [x] **Hit-test**: `drawHitFromCache(40)` (ALPHA_HIT_THRESHOLD), opakovaný klik cykluje překryvy top-down s wrapem (`getAllIntersections` + lastClickRef, tolerance 4 px), status bar „Klik vybere: … · N vrstev pod kurzorem"
- [x] **Ergonomie**: sbalitelné Section karty (badge, headerExtra, localStorage persist u sub-karet), 13,5–14 px text, 32 px touch targety, tooltipy všude, miniatury vrstev (40 px, URL sdílená s Konva → browser cache) i projektů (`GET /api/projects/{id}/thumbnail`, disk cache thumb.png, 404 pro prázdný projekt); 3 layout bugy chycené E2E opraveny (project-actions display:none místo opacity, sidebar scroll místo flex-grow squash, ▲▼ horizontálně + akce na hover)
- [x] **Spike pryč**: přepínač odstraněn, SpikeStage jen na `/spike` (path-gate)
- [x] **Textové vrstvy v1**: `Layer.is_text/text_score/text_original/text_current`; `POST /detect-text` = SAM koncept **„letters"** (`DETECT_TEXT_PROMPT` — „text" vrací 0 masek, empirie!) nad kompozitem + atribuce přes `layer_mask_on_canvas` (podíl alfy pod union maskou, práh 0,35; reálná separace 0,74–0,98 vs. 0,00–0,04); auto-detekce po dekompozici + tlačítko „🔤 Detekovat text"; skupina „Textové vrstvy" + badge TEXT; `POST /layers/{id}/rewrite-text` = pinnuté prompt templaty + **pásová maska** (bbox glyfů + 0,4× výška řádku, MERGE) + **SAM „letters" re-cutout** alfy na nové glyfy (+$0.005; bez něj se protínající pásy sousedních text vrstev zapékají navzájem); refaktor `_ai_edit_apply` (sdílený ai-edit/rewrite, `mask_img` kw)
- Testy: 207 pytest (m6ux1: 12; band + re-cutout pinnuté). Limity: qwen typa na malém textu (retry), vícerádkový pás umí text přidat místo nahradit (přepisovat po řádcích), I2L duchové kopie textu (skrýt), rewrite fixně qwen (flux-fill kandidát), text_original bez OCR.
...
> 7. Zbytky z M6-UX1: flux-fill přepínač v Text sekci, auto-detekce duchových text kopií z I2L (skrýt/inpaint-behind), per-řádkové text bboxy (přepis vícetextové vrstvy po řádcích), OCR pro text_original, thumbnaily klipů v Klipy sekci
